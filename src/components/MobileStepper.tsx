...
export function MobileStepper({ currentStep, onStepChange, onCopy }: MobileStepperProps) {
  const steps: Step[] = [
    {
      id: 1,
      title: 'اطلاعات گزارش',
  ...
  return (
    <div className="lg:hidden mb-8 px-2">
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              ...
              {index < steps.length - 1 && (
                <div className="w-6 md:w-8 lg:w-10 h-1 mx-0.5 md:mx-1 relative flex-shrink-0">
      ...
      <div className="flex justify-center items-center mb-6 px-1 overflow-x-auto no-scrollbar">
          ...
          {steps.map((step) => (
            <button
              ...
              className={`
                ...
                px-2 py-1.5 md:px-3 md:py-2 rounded-lg md:rounded-xl 
                transition-all duration-200
                min-w-[50px] md:min-w-[60px] flex-shrink-0
                ${isCurrentStep(step.id)
      ...
      <div className="flex justify-between items-center gap-3 md:gap-4 mt-8 px-2">
        {currentStep > 1 && (
          <button
            onClick={() => onStepChange(currentStep - 1)}
            className={`
              flex items-center justify-center gap-1.5 md:gap-2 
              px-4 md:px-5 py-2.5 md:py-3 rounded-lg md:rounded-xl 
          ...
          >
            ...
            <span className="text-sm md:text-base">مرحله قبل</span>
          </button>
        )}
        ...
        {currentStep < 5 && (
          <button
            ...
            `}
          >
            <span className="text-sm md:text-base">مرحله بعد</span>
            <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" />
          </button>
        )}
        
        {currentStep === 5 && onCopy && (
          <button
            onClick={onCopy}
            className={`
              flex items-center justify-center gap-1.5 md:gap-2 
          ...
          >
            ...
            <span className="text-sm md:text-base">تکمیل گزارش ✓</span>
          </button>
        )}
      </div>
    </div>
